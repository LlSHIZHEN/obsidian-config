...
**例外：你自己写的片段会被打包。** 脚本对每个片段回上游比对 sha256 来判定归属，
**不是按文件名猜的**：

- 与上游同路径文件逐字节一致 → 第三方作品，**不打包**，安装时回上游取
- 与上游不一致或上游根本没有 → **你自己的作品**，打包进 `local/snippets/`

这个区分不是洁癖，而是必须的。起初脚本假定"所有片段都来自主题仓库"，结果
把一个本地自写的 `text-contrast-fix.css` 编造了指向 AnuPpuccin 的下载地址——
后果是**公开仓库里把你的原创错标成他人作品，且新电脑恢复时 404、自定义静默丢失**。
自有作品不涉及第三方许可，打包它既合规又保真，而且 `--offline` 也能恢复。
